// scripts/backtest/satelliteStopLoss/backtestGrid.ts
// 투더문(위성) 31종 매도규칙 그리드 백테스트 — 연구 전용(앱/백엔드 무접촉).
//
// coreStopLoss/backtestGrid.ts 와 **같은 엔진**(lib/sellRuleEngine, lib/movingAverage,
// lib/splice, lib/yahooData)을 상대경로로 그대로 재사용한다. 로직 복제 없음.
// 다른 점은 대상 유니버스와 사이징 전제뿐이다:
//   - 대상: 사용자가 지정한 투더문(위성) 31종 (종목명만 근거, 보유수량/매수가 일체 미사용)
//   - 비중: **전 종목 균등**(100/N %) — 사용자 명시 지시
//   - 허용손실: 총자산 1%/종목 → 손절폭 = 1% / (1/N) = N%  (균등비중이므로 전 종목 동일)
//   - 투자예산: 100%(예비현금 없음)
//
// 지평별 레그 구성이 달라진다: 상장이 늦어 그 지평의 시작 시점에 이동평균(최대 50일) 워밍업을
// 마치지 못한 종목은 **그 지평에서만 제외**하고 리포트에 명시한다(침묵 제외 금지).
// 개별주 유니버스라 의미 있는 프록시가 없으므로 접합(splice)은 쓰지 않는다.
//
// 그리드: 익절배수 4 × 마지막매도선 3 × 불타기 5 = 60조합 × 지평 4개 = 240런.
//
// 실행: npx --yes tsx scripts/backtest/satelliteStopLoss/backtestGrid.ts
...
async function main(): Promise<void> {
  ...
  console.log('투더문(위성) 31종 매도규칙 그리드 백테스트 — 균등비중 + 총자산 1% 최대손실');
  console.log(
  ...
  const resolved = await resolveUniverse(ALL_ENTRIES, DATA_START, END);
  ...
  const report = {
    ...
    universe: 'satellite(투더문) 31종 — 사용자 지정 종목명 기준, 균등비중',
    config: {
    ...
    caveats: [
      ...
      '균등비중이므로 손절폭은 전 종목 동일하다: 1%(허용손실) / (1/N) = N%. N은 지평별 편입 종목 수라 지평마다 손절폭이 달라진다.',
      '지평 시작 시점에 MA 워밍업(최장 50일)을 못 마친 종목은 그 지평에서만 제외했다(horizons[].excludedLegs 참조). 개별주라 의미 있는 프록시가 없어 접합은 쓰지 않았다.',
      ...
      '공통 캘린더가 한국·미국 거래일의 합집합이라 이동평균 기간은 "한 시장의 거래일"이 아니라 합집합 캘린더 일수 기준이다(결측일은 직전 종가 carry-forward).',
      '미국 종목은 일별 USD/KRW(Yahoo KRW=X)로 환산했다 — 환율 변동이 손익에 그대로 반영된다.',
      'Yahoo 수정종가(배당·분할 반영) 기준. 무배당/최근상장 종목은 조정비율이 1.0(flat)으로 나오는 것이 정상이다.',
      '모든 체결은 종가 기준이며 슬리피지·거래비용·세금은 반영하지 않았다. 유동성이 낮은 소형주가 다수 포함되어 실제 체결가 괴리가 클 수 있다(gate0_report.json의 illiquid 플래그 참조).',
      '부분익절 매도도 ClosedTrade로 기록되므로 승률/손익비는 "청산 건"이 아니라 "매도 건" 기준이다.',
      '31종 전체를 동시에 균등 보유하는 것은 실제 운용 이력이 아니라 가정이다(생존편향: 현재 보유 중인 종목만으로 과거를 재구성했다).',
      'horizons[].buyHold는 같은 레그 집합·같은 구간을 매도규칙 없이 균등 매수 후 보유한 기준선이다. 조합 성과는 반드시 이 기준선과 비교해서 읽어야 한다.',
      '손절폭이 N%(25~29%)로 매우 넓어 실제로는 마지막매도선(MA 하향돌파)이 항상 먼저 발동한다 — 손절 체결 0건은 버그가 아니라 균등비중×1% 규칙의 산술적 귀결이다.',
    ],
  };

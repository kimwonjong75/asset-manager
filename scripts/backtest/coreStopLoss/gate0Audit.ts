// scripts/backtest/coreStopLoss/gate0Audit.ts
// Gate 0 데이터 감사 — 코어(정적배분) 손절 백테스트를 시작하기 전에
// 목표 배분 14종 + 장기확장 프록시 6종이 (1) 수정(총수익) 데이터인지
// (2) 상장일/구간/유동성/구멍이 백테스트에 쓸 만한지 검사한다.
// 연구 전용(앱/백엔드 무접촉, Yahoo v8 직접 조회).
//
// 실행: npx --yes tsx scripts/backtest/coreStopLoss/gate0Audit.ts
...
async function main(): Promise<void> {
  ...
  console.log('Gate 0 데이터 감사 — 코어(정적배분) 손절 백테스트');
  ...
  for (const t of TRACKS) {
    ...
    for (const r of trackRows) {
      ...
      console.log(
        '  ' +
          pad(`${mark} ${r.symbol}`, 14) +
          pad(r.label + (r.probe ? '·프로브' : ''), 26) +
          pad(r.firstDate ?? '—', 12) +
          pad(r.lastDate ?? '—', 12) +
          pad(String(r.n), 7) +
  ...
  // ─── 최초거래일(상장일 확인) 표 ─────────────────────────────
  // 백테스트 가능 지평을 결정하는 핵심 정보이므로 별도로 뽑아준다.
  console.log('\n  최초거래일(Yahoo 기준, 상장일 근사) — 늦은 순:');
  const byFirst = rows
    .filter(r => r.ok && r.firstDate)
    .sort((a, b) => (a.firstDate! < b.firstDate! ? 1 : -1));
  for (const r of byFirst) {
    const years = (daysBetween(r.firstDate!, r.lastDate ?? END) / 365.25).toFixed(1);
    console.log(
      '    ' +
        pad(r.symbol, 12) +
        pad(r.label, 24) +
  ...
  // 실제 보유 14종(프록시 확장 제외) 기준 공통 시작일 = 손절 백테스트 하한.
  const realRows = rows.filter(r => r.ok && r.track !== 'proxy-extension' && r.firstDate);
  const realCommon = realRows.reduce<string | null>(
    (acc, r) => (acc === null || r.firstDate! > acc ? r.firstDate! : acc),
    null
  );
  console.log(
    `\n  실제 보유 14종 공통 시작가능일: ${realCommon ?? '—'} (가장 늦게 상장한 종목 기준)`
  );
  const proxyRows = rows.filter(r => r.ok && r.track === 'proxy-extension' && r.firstDate);
  const proxyCommon = proxyRows.reduce<string | null>(
    (acc, r) => (acc === null || r.firstDate! > acc ? r.firstDate! : acc),
    null
  );
  console.log(`  프록시 대체 시 공통 시작가능일(확장 지평): ${proxyCommon ?? '—'}`);
  ...
  const report = {
    ...
    summary: {
      ...
      flat,
      realCommonStart: realCommon,
      proxyCommonStart: proxyCommon,

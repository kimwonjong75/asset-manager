...
check("tab=today → today(실제 탭, P3부터 dashboard 자리표시자 매핑 폐지)", parseDeepLink('?tab=today'), { tab: 'today', assetRef: null });
check('tab=today + asset(id) 형태', parseDeepLink('?tab=today&asset=abc123'), { tab: 'today', assetRef: { kind: 'id', id: 'abc123' } });

...
- `services/`
  - `priceService.ts` 가격 데이터 소스(Cloud Run/Upbit)
  - `geminiService.ts` 환율 조회 등(선택/폴백 용)
...
2. 시세 소스 변경: 주식은 Cloud Run, 코인은 Upbit로 교체 (`services/priceService.ts`)
3. 포트폴리오 정렬 고도화: 수익률 헤더 클릭 시 순환 정렬
4. 대시보드 차트 보강: 투자 원금/총 평가액 라인 & 툴팁 개선
5. 문서화: 프로젝트 개요/구조/설치/환경/데이터 소스/UX 정리

## 주의 사항
- Cloud Run API는 `tickers`가 비어있으면 오류를 반환합니다. 배치 요청 시 항상 유효한 틱커/거래소 페어를 전달하세요.
- 환율 키(Gemini)는 선택 사항이며, 제공하지 않는 경우 앱 내부 기본값/사용자 입력을 사용합니다.

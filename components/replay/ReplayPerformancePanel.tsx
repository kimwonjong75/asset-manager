                ...
                {performance.map(p => (
                  <tr key={p.key} className="border-t border-gray-700/40">
                    <td className="py-1.5 pr-2">
                      <span className={`text-xs mr-1 px-1 rounded whitespace-nowrap ${p.kind === 'guru' ? 'bg-violet-500/20 text-violet-300' : 'bg-sky-500/20 text-sky-300'}`}>
                        {p.kind === 'guru' ? '구루' : '알림'}
                      </span>
                      <span className={`text-xs mr-1 ${p.action === 'sell' ? 'text-down' : 'text-up'}`}>
                        {p.action === 'sell' ? '매도' : '매수'}
                      </span>
                    ...
                    <td className={`py-1.5 px-1 text-right font-mono ${p.avgRet20 != null && p.avgRet20 >= 0 ? 'text-up' : p.avgRet20 != null ? 'text-down' : 'text-gray-500'}`}>
                      {pct(p.avgRet20)}
                    </td>

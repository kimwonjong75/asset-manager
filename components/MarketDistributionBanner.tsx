...
// - 3회 이상인 지수만 표시: 노랑(3) / 주황(4) / 진한 주황+경고 아이콘+굵은 문구(5+)
//   (Stage B 색 규약: 위험은 주황+아이콘+문구 — 빨강은 '오름' 전용이라 쓰지 않는다)
// - 오닐 원의도: 시장 지수 디스트리뷰션 누적 = 시장 전체 위험 신호

import React from 'react';
import { Info, OctagonAlert, type LucideIcon } from 'lucide-react';
import {
      ...
      {visible.map(entry => {
        ...
        return (
          ...
          >
            ...
            <span className={`text-xs sm:text-sm ${SeverityIcon ? 'font-bold' : ''}`}>{styles.message(entry.count)}</span>
          </div>
        );
      })}
      {/* 면책(투자자문 아님)은 홈 하단 한 줄로 통합 */}
      <p className="text-xs text-gray-500 px-1">
        지수 디스트리뷰션은 시장 분위기 진단 도구입니다.
      </p>
    </div>
  );
};

...
const STEPS: WizardStep[] = [
  ...
  },
  {
    ...
    body: (
      <>
        브라우저에서 <code className="bg-gray-800 px-1 rounded text-[11px]">&lt;웹앱 URL&gt;?action=kakao-auth</code> 를 열어
  ...
  return (
    <div className={`bg-gray-900/50 border border-gray-700 rounded-lg p-3.5 space-y-3 ${className}`}>
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] text-gray-400">{doneCount}/5 완료</span>
      </div>
          ...
          <li key={i} className="border-t border-gray-800 pt-2.5 first:border-t-0 first:pt-0">
            <label className="flex items-start gap-2 cursor-pointer select-none">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-gray-400 leading-relaxed mt-0.5">{step.body}</p>
                <p className="text-[11px] text-amber-500/80 mt-1">{step.hint}</p>
              </div>
            </label>
          </li>
        ))}

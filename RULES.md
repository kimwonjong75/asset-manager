...
| 파일 | 책임 | 의존 |
|------|------|------|
| `DashboardView.tsx` | 대시보드 탭 — 최상단에 `MarketDistributionBanner` (시장 지수 디스트리뷰션 경고, 모든 지수 safe면 자동 미렌더). `GoldPremiumWidget` 아래 **`RiskCalculatorCard`**(접힘형 풀폭). **Phase 5(신호 다이어트): `GuruSignalCard`는 항상 상단 위치 유지** — `showGuruSignalsProminently`가 true면 기존처럼 펼쳐 크게, false(기본)면 **같은 자리에서 접이식**(`collapsible`+`defaultCollapsed`, storageKey `asset-manager-guru-card-open`). 바로 아래 **`ReferenceIndicatorsSection`**(참고 지표, 구루 카드 제외·리스크 매트릭스만). **`ProfitLossChart`는 접이식 기본 접힘**(collapsible+defaultCollapsed, storageKey `asset-manager-profitloss-open`). **`TopBottomAssets`(자산별 성과 요약)는 대시보드에서 렌더 제거**(컴포넌트/훅은 보존). 계산/차트/발화 무변경(표시·레이아웃만). **계정 뷰 필터(`ui.accountView`)**: `viewAssets`(matchesOwnerFilter)를 요약 통계·DashboardControls·AllocationChart·CategorySummaryTable(비중 분모=`viewTotalValue`)·ProfitLossChart에 적용. **자산구분 필터 시맨틱**: 숫자=**코어 버킷의 해당 카테고리만**, `'SATELLITE'`=투더문 버킷 전체 — 배분 차트/자산군별 요약의 코어·투더문 분해와 일치(투더문 종목은 카테고리 필터에 안 잡히고 '투더문' 필터로만). **의도적 예외 — 원본 assets 유지**: 매도통계(allSellRecords — SellRecord에 owner 없음, 통합 기준 1차 한계)·RebalancingTable(뷰 무관, useRebalancing이 내부에서 유선 상시 제외) | `PortfolioContext`, `useGlobalPeriodDays`, `types/owner`, `GoldPremiumWidget`, `MarketDistributionBanner`, `RiskCalculatorCard`, `GuruSignalCard`, `ReferenceIndicatorsSection`, `ProfitLossChart`, `AllocationChart`, `CategorySummaryTable`, `RebalancingTable` |
...
| `DashboardControls.tsx` | 카테고리 필터 드롭다운 + 환율 입력. **필터 값 `number \| 'ALL' \| 'SATELLITE'`** — 카테고리 목록 뒤에 "🚀 투더문 (위성)" 옵션(버킷 필터, 카테고리 아님) | `ExchangeRateInput`, `getAllowedCategories` |

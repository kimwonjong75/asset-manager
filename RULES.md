...
| `useActionQueue.ts` | **실행 큐 오케스트레이션 (Phase 2b-2)**. **`refreshActionQueue()`(명시적 호출 전용 — 마운트 자동 effect 없음, "보이지 않는 쓰기" 방지)**: 오픈 포지션+관심종목 터틀후보(`isTurtleCandidate`)만 대상 → 종목 메타(현재가=asset.priceOriginal/관심종목 priceOriginal·통화·거래소) 해석 → **최소 기간 OHLCV fetch**(`turtleHistoryWindow`≈105일, 주식/코인 분기) → `assembleMarketInput` → `buildTurtleActions` → `reconcileActionQueue` → **변화(생성>0 or 스누즈만료) 있을 때만 `updateActionQueue` 저장**. 사용자 행동(보이는 쓰기): `markDone(id, sellRecordId?)`/`markSkipped(id, reason)`(사유 필수·공백 무시)/`snoozeAction(id, days=1)`(snoozeCount++). 반환 `{actionQueue, refreshActionQueue, markDone, markSkipped, snoozeAction, isRefreshing, refreshError}`. **아직 어떤 컴포넌트도 미사용(2b-3 UI에서 연결) → 자동 실행 없음** | `PortfolioContext`(data/updateActionQueue), `historicalPriceService`, `actionQueueGenerator`, `turtleMarketData` | (2b-3 ExecutionView) |
...
| `execution/ExecutionView.tsx` | **"실행 큐(오늘의 주문서)" 탭 (신규 `'execution'` 탭, Phase 2b-3)** — `useActionQueue` 연결, 렌더 전용. **화면 진입 자동 refresh 없음** — "오늘 주문 생성" 버튼 클릭 시에만 `refreshActionQueue()`(명시적, 보이지 않는 쓰기 방지). 대기(pending/snoozed) 주문을 **에스컬레이션(3일+/스누즈2회+=주의, 7일+/3회+=경고) 높은 순→오래된 순** 정렬해 카드로: kind 뱃지(신규매수/불타기/손절/청산/리밸런싱/정리)·종목·수량·기준가·근거(`reasonText`)·미실행 일수. 카드 액션 **3개: 실행 완료(`markDone`)·건너뜀(인라인 textarea **사유 필수**·`markSkipped`)·내일(`snoozeAction`)**. 위성 예산 0이면 안내. **범위: UI+상태전환만 — done은 "증권사 실행 후 완료 표시"(매수/매도 모달 prefill·SellRecord 연결은 2b-4)**. 기본 탭 변경 안 함 | `useActionQueue`, `PortfolioContext`(turtleSettings), `actionQueueGenerator`(escalation), `types/actionQueue` ← `App.tsx` |
...
| `turtleExecutionParity.ts` | **터틀 저장성공-후 상태전이(Phase 2b-4b-2c/2d) 골든 테스트**(25 단언). applyEntry/Pyramid/Close(포지션 없음·closed=불변)·completeQueueItem(done+linkedSellRecordId·매수는 link 없음)·exitReasonForKind. 원본 불변. 운영 함수만 호출 | `npm run test:turtleexec` |

...
| `useSignalReplay.ts` | **신호 리플레이 오케스트레이션** — OHLCV fetch(주식/코인 분기, 10년, requestId 레이스 가드, 실패 폴백) + 종목검색(debounce 250ms) + 타임라인 계산(`buildReplayTimeline`, 한 틱 양보 후 동기 빌드 — 252일/단일종목 기준 충분, 느리면 후속 청크화) + as-of 네비(slider/click/이전·다음 날/이전·다음 신호/최신) + 모드(replay/review)·윈도(6M/1Y/2Y/3Y) + 샌드박스 오버라이드 state(P3 UI 연결, **1차 빈 상태**). **유효 규칙=`applyRuleOverrides(knowledgeBase.rules, sandbox)`(1차 영구 오버라이드 없음). 라이브 구루 신호/KnowledgeBase 절대 불변** | `signalReplay`, `ruleOverrides`, `historicalPriceService`, `symbolListService`, `PortfolioContext`(data.knowledgeBase, ui.alertSettings.rules) | `SignalReplayView` |
...
| `tests/signalReplayParity.ts` | 독립 실행 (`npm run test:replay`, 31단언). 신호 리플레이 회귀 — **룩어헤드 방지**(full vs asOf까지 자른 시리즈 진단 동일, outcome만 상이) + `boundaryDistance`(between/음수/0) + `applyRuleOverrides`(leafId 패치·enabled=false 제거·비파괴) + `flattenLeaves` 안정 id + `buildReplayTimeline` 동작 + **마커 구루 게이팅**(markers.guruCount>0·signalDates가 구루 마커로만 뒷받침). 합성 OHLCV(네트워크 없음). 운영 함수만 호출(계산식 복제 금지) |

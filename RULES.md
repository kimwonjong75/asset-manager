...
| `turtleReview.ts` | **터틀 자동 검토 요약 (자동 검토 Phase A/B, 순수 함수)** — 실행 큐 대기 현황+진단 결과를 상단 배지·브리핑 표시용 숫자로 요약(읽기 전용, 생성·저장 경로 무접촉). `summarizeActiveQueue(queue, today)`→`{activeCount(pending/snoozed만), escalatedCount(actionEscalationLevel≥1 — 3일+/스누즈2회+)}`. `summarizePreview(diag|null)`→kind별 분해(previewEntry/Pyramid/Stop/Exit) — **previewCount는 `diag.generatedCount` 그대로 사용**(≡buildTurtleActions 길이, 기존 앵커) + 분해 합 일치는 테스트가 강제. `buildTurtleReviewSummary({...})`→`TurtleReviewSummary`(**actionableCount=activeCount+previewCount**[진단이 대기 주문을 duplicate-pending으로 배제하므로 이중 카운트 없음], turtleCandidateCount/budgetMissing/isChecking/reviewPending/reviewFailed/checkedAt 'HH:MM'). 회귀 `tests/turtleReviewParity.ts`(42단언, `npm run test:turtlereview` — 프리뷰≡생성 앵커+게이트 실행 축 하위호환 포함) | `types/actionQueue`, `actionQueueGenerator`(actionEscalationLevel) ← `useTurtleActionReview`, `types/store`(TurtleReviewSummary), `AlertPopup`(실행 카드), `App.tsx`(배지) |
...
| 파일 | 책임 | 의존 |
|------|------|------|
| `DashboardView.tsx` | 대시보드 탭 — 최상단에 `MarketDistributionBanner` (시장 지수 디스트리뷰션 경고, 모든 지수 safe면 자동 미렌더). `MarketOverviewBar`(금 김치 프리미엄 + 환율 통합 바) 아래 **`RiskCalculatorCard`**(접힘형 풀폭). **Phase 5(신호 다이어트): `GuruSignalCard`는 항상 상단 위치 유지** — `showGuruSignalsProminently`가 true면 기존처럼 펼쳐 크게, false(기본)면 **같은 자리에서 접이식**(`collapsible`+`defaultCollapsed`, storageKey `asset-manager-guru-card-open`). 바로 아래 **`ReferenceIndicatorsSection`**(참고 지표, 구루 카드 제외·리스크 매트릭스만). **`ProfitLossChart`는 접이식 기본 접힘**(collapsible+defaultCollapsed, storageKey `asset-manager-profitloss-open`). **`TopBottomAssets`(자산별 성과 요약)는 대시보드에서 렌더 제거**(컴포넌트/훅은 보존). 계산/차트/발화 무변경(표시·레이아웃만). **계정 뷰 필터(`ui.accountView`)**: `viewAssets`(matchesOwnerFilter)를 요약 통계·DashboardControls·AllocationChart·CategorySummaryTable(비중 분모=`viewTotalValue`)·ProfitLossChart에 적용. **자산구분 필터 시맨틱**: 숫자=**코어 버킷의 해당 카테고리만**, `'SATELLITE'`=투더문 버킷 전체 — 배분 차트/자산군별 요약의 코어·투더문 분해와 일치(투더문 종목은 카테고리 필터에 안 잡히고 '투더문' 필터로만). **의도적 예외 — 원본 assets 유지**: 매도통계(allSellRecords — SellRecord에 owner 없음, 통합 기준 1차 한계)·RebalancingTable(뷰 무관, useRebalancing이 내부에서 유선 상시 제외) | `PortfolioContext`, `useGlobalPeriodDays`, `types/owner`, `MarketOverviewBar`, `MarketDistributionBanner`, `RiskCalculatorCard`, `GuruSignalCard`, `ReferenceIndicatorsSection`, `ProfitLossChart`, `AllocationChart`, `CategorySummaryTable`, `RebalancingTable` |
...
| `execution/TurtleSettingsPanel.tsx` | **터틀(위성) 최소 설정 패널 (Phase 2b-4c, 렌더 전용)** — 실행 큐 상단. `satelliteBudgetKRW`(모든 사이징의 분모, 0=fail-closed로 진입 주문 미생성) 입력 + **"총자산의 10%로 설정"**(`derived.totalValue`×0.1, 입력만 채움) + **명시적 저장**(`actions.updateTurtleSettings({...settings, satelliteBudgetKRW})` — 키 입력마다 저장 금지). 저장 후 `✓ 저장됨` 인라인 표시(플로팅 토스트 아님). `riskPerUnitPct`/`maxUnitsPerPosition`/`maxTotalRiskPct`/`positionValueCapPct`/`entryLookback`/`exitLookback`는 **기본값 유지·읽기 표시만**. 예산 0이면 앰버 경고. 외부 예산 변경 시 입력 동기화(useEffect). **「오늘 주문 자동 생성」 토글(자동 검토 Phase C)**: `settings.autoGenerateQueue` 스위치(role=switch) — 토글은 명시적 사용자 행동이라 **즉시 `updateTurtleSettings` 저장**(예산 입력의 명시적 저장 버튼과 구분), 기본 OFF, 문구로 "실제 매수/매도가 아니라 주문 목록 생성까지만 자동" 명시 | `PortfolioContext`(data.turtleSettings/derived.totalValue/updateTurtleSettings) ← `ExecutionView` |
...
| `scripts/backtest/conditionalChannel/simulator.ts` | **신호·체결·포지션·비용 순수 엔진**(`console.*`/`Math.random` 금지). 종목별 실제 거래일 인덱스 룩백(합집합 달력 아님)·현재바 제외 Donchian(donchian utils 재사용)·`isEntrySignal`(close>H_N 엄격)·`commonStartIndex`/`naturalStartIndex`·ATR 워밍업 firstValidIndex 슬라이스 우회·`resolveFillIndex`(종가확인→t+1시가·지연스트레스)·`stopFill`(§9-8 갭시가/장중손절+슬리피지)·`tradeCost`(티어 0/1/2×, **세금은 티어무관 실세율**)·`positionSize`(0.5%·25%캡·정수내림)·`proRataRiskAllocation`(같은날 다중신호 비례배분·선착순 금지)·`advParticipationOk`(60일 중앙거래대금 5%초과→미체결 reject)·`resolveDelisting`(대가/수익률·불명시 제외)·`closeOpenPosition`(FORCED/MTM/EXCLUDE 3방식)·`simulateSecurity`(단일종목)·`simulatePortfolio`(자본공유·합집합달력은 순서배정만). **Bug1: pending-order 결제 모델**(채널청산·신규진입은 fillDate에만 현금/포지션 변형·신호일 원장 불변·손절/상장폐지는 당일 즉시·openRisk는 결제완료 포지션만·equityCurve에 cash 노출). **Bug2: `PortfolioSecurity.group` 정적필드 제거→`monthlyFlags[]`**·진입 시점 `resolveFrozenClassification`로 그룹 동결·미분류는 ADAPTIVE/REVERSE에서 UNFILLED(`UNCLASSIFIED_GROUP`)·ALL_20/ALL_55(그룹무관)만 명목그룹으로 진입 | 설계선택: 손절 우선(가장 보수적 손익), ADV 초과=미체결, ZERO=세금 유지, 그룹민감성=entryLookback(A)≠entryLookback(B) |

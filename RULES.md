...
| `tests/signalReplayParity.ts` | 독립 실행 (`npm run test:replay`, 84단언). 신호 리플레이 회귀 — **룩어헤드 방지**(full vs asOf까지 자른 시리즈 진단 동일, outcome만 상이) + `boundaryDistance`(between/음수/0) + `applyRuleOverrides`(leafId 패치·enabled=false 제거·비파괴) + `flattenLeaves` 안정 id + `buildReplayTimeline` 동작 + **마커 구루 게이팅**(markers.guruCount>0·signalDates가 구루 마커로만 뒷받침) + **P2 판정 저장**(verdictKey 키구분·upsert 수정/추가/비파괴·놓친매수 비신호일 태깅·삭제 day vs rule 분리·안전파싱) + **P2 사례**(collectPerRuleResults eligible&&matched만·computeResultMetrics avgRet20·buildVerificationCase rulesetHash 일치·**종목/기간 밖 판정 필터·규칙별 판정 보존**·diffSignalDates/diffCaseResults added/removed·upsertCase 최신우선·removeCase·parseCases 폐기). 합성 OHLCV(네트워크 없음). 운영 함수만 호출(계산식 복제 금지) |

...
| `turtle.ts` | **터틀 전략 포지션 모델 + 설정 (Phase 1, 90/10)** — `TurtleUnit`(fillDate/fillPrice/quantity/nAtFill[원통화]/**`fxRateAtFill?`**[감사용, 손절가 역환산 금지]), `TurtlePosition`(units[]+공통 stopPrice[원통화]+entryDonchianHigh+status/exitReason), `TurtleSettings`(satelliteBudgetKRW=사이징 분모, riskPerUnitPct=0.5, maxUnitsPerPosition=2[상한4], entryLookback=55, exitLookback=20, stopMultipleN=2, pyramidStepN=0.5, maxTotalRiskPct=12, positionValueCapPct=25, drawdownScalingEnabled), `DEFAULT_TURTLE_SETTINGS`(예산 0=fail-closed). 엔진 산출물 타입: `TurtleUnitSize`(`positionValueKRW`)/`TurtleEntryProposal`(stopPrice 원통화·riskKRW·positionValueKRW·`fxRateUsed`)/`TurtleEntryDecision`(+`EntryRejectReason` 5종)/`TurtlePyramidProposal`/`TurtleSellProposal`/`TurtleTotalRisk`. **통화 규약(D6): 가격-공간(stop/entry/donchian/N)=원통화 저장, 돈-공간=KRW, FX는 사이징·리스크만. 1% 손실규칙 계좌=위성예산(코어 90%엔 손절 없음). Phase 2가 proposal→ActionItem 변환** | `utils/turtleEngine`, (Phase 2 useActionQueue/ExecutionView) |

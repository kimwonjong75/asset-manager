...
| `RebalancingTable.tsx` | **2단 리밸런싱 표** — ① 전략 배분(코어 vs 투더문) ② 코어 자산배분(카테고리별, 코어 합계 기준) ③ 투더문 현황(위성 보유 종목 참고, 합산 한 덩어리). 공용 `TierTable` 서브컴포넌트로 ①②를 렌더(목표비중 입력·합계·편차 색상), 목표총액 입력·저장은 공유. `useRebalancing` 반환의 `bucket`/`core`/`satelliteHoldings`/`hasSatellite` 사용. **밴드 이탈 안내(Phase 4a-2)**: ② 코어 표 아래에 `bandDeviations`(비어있지 않을 때만) 앰버 안내 섹션 — 카테고리별 현재%→목표%·편차%p·KRW 부족/초과 금액. **표시 전용(주문·버튼·수량·종목 없음)·"안내" 톤·코어 카테고리 기준 명시**. 0건이면 미표시. **③ 대표 매수 종목 지정(Phase 4b-1)**: `InstrumentRow` 서브컴포넌트로 코어 카테고리별 대표 종목 지정/변경/삭제(티커 input+datalist[코어 보유]+거래소 select[`getExchangesForCategory`]). 티커가 보유와 일치하면 이름·통화 자동 채움. **로컬 편집→저장하기로 영속(handleSave spread 보존)·주문/수량/가격fetch 없음(4b-2+)**. 투더문은 ④로 | `useRebalancing`(categoryInstruments/handleInstrumentChange/coreHoldingsByCategory/bandDeviations), `PortfolioContext`, `utils/bucketRebalancing`, `types/category`(getExchangesForCategory) |

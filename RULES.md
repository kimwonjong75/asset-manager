...
| `AlertDiagnosticsPanel.tsx` | **알림 진단 패널 (5B-①) — "왜 이 알림이 떴나/안 떴나"**. `useAlertDiagnostics` 렌더만. **상단: 팝업 전달 상태 배너**(자동 브리핑 표시 예정/미표시 + 사유 + 발화 규칙 수 — 규칙 발화와 **직교 축으로 분리 표시**해 "규칙 미충족"과 "팝업 꺼짐/오늘 표시함" 혼동 방지). 종목 `<select>`(보유/관심[매수규칙만] optgroup) → 규칙별 행(매도/매수 뱃지 + `status.label`을 tone 색으로 + "조건별 보기 ▾" → 필터별 ✓/✗/— 실제값vs기준, **품질 저하는 '데이터 일부'(amber)/'데이터 없음' 캐비엇**). 빈 상태/범례. 모바일 세로 스택(격리 프리뷰 검증: 가로 오버플로 없음, partial 축 노출 확인) | `useAlertDiagnostics`, `types/alertDiagnostics` |

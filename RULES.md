...
| `useGoogleDriveSync.ts` | Google Drive 저장/로드, **`needsReAuth` 상태 관리**(세션 만료 시 데이터 유지+배너), 인증 상태 변경 콜백 등록. **`LoadedData.knowledgeBase`**(로드 파싱) + **`autoSave` 9번째 인자 `knowledgeBase`** → `exportData`에 `knowledgeBase` 키로 직렬화(categoryStore와 동일 패턴). **90/10(Phase 2b): `LoadedData`에 `actionQueue`/`turtlePositions`/`turtleSettings` + `autoSave` 10~12번째 인자 + `exportData` 3키** (배열은 `Array.isArray` 가드). 새 단일 JSON 키 추가 시 LoadedData/load파싱/autoSave시그니처/exportData 4곳 동시 반영 | `googleDriveService`, `lz-string`, `types/knowledge`, `types/turtle`, `types/actionQueue` | `usePortfolioData` |

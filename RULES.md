...
| `execution/ExecutionView.tsx` | **"실행 큐(오늘의 주문서)" 탭 (신규 `'execution'` 탭, Phase 2b-3)** — `useActionQueue` 연결, 렌더 전용. **화면 진입 자동 refresh 없음** — "오늘 주문 생성" 버튼 클릭 시에만 `refreshActionQueue()`(명시적, 보이지 않는 쓰기 방지). 대기(pending/snoozed) 주문을 **에스컬레이션(3일+/스누즈2회+=주의, 7일+/3회+=경고) 높은 순→오래된 순** 정렬해 카드로: kind 뱃지(신규매수/불타기/손절/청산/리밸런싱/정리)·종목·수량·기준가·근거(`reasonText`)·미실행 일수. 카드 액션 **3개: 실행하기/실행 완료·건너뜀(인라인 textarea **사유 필수**·`markSkipped`)·내일(`snoozeAction`)**. **터틀 kind(`TURTLE_ENTRY/PYRAMID/STOP/EXIT`)의 실행 버튼은 `actions.openTurtleExecution(item)`으로 `TurtleExecuteModal`을 염**(라벨 '실행하기') → 모달 저장 성공 시에만 done+lifecycle 커밋. **비터틀 kind는 기존 `markDone`(표시만, 라벨 '실행 완료')**. 모달은 이 뷰에서 렌더하며 `executeTurtleAction`을 prop으로 전달(useActionQueue 1회 인스턴스화). **상단에 `TurtleSettingsPanel`(위성 예산 입력, Phase 2b-4c)** 노출. 기본 탭 변경 안 함 | `useActionQueue`, `PortfolioContext`(openTurtleExecution), `actionQueueGenerator`(escalation), `types/actionQueue`, `TurtleExecuteModal`, `TurtleSettingsPanel` ← `App.tsx` |

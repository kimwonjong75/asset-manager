...
| 파일 | 책임 | 수정 시 영향 범위 |
|------|------|------------------|
| `portfolioCalculations.ts` | 포트폴리오 계산 유틸. **`mapToNewAssetStructure`가 로드 파이프라인의 자산 기본값 단일 보정점** — categoryId fallback + **전략 버킷 기본값 보정(`bucket = bucket ?? 'CORE'`, 레거시=전부 코어)**. 모든 Drive 자산이 이 함수를 통과(`usePortfolioData` 로드)하므로 신규 Asset 기본 필드는 여기서 채울 것. **단 대청소 필드(`cleanupTag`/`excludedFromCleanup`, Phase 3b)는 spread로 보존만 하고 기본값 강제 주입 금지**(미검토≠keep, 누락=false) | 전역 (계산 결과 변경) |
...
| 파일 | 책임 | 의존 |
|------|------|------|
| `DashboardView.tsx` | 대시보드 탭 — 최상단에 `MarketDistributionBanner` (시장 지수 디스트리뷰션 경고, 모든 지수 safe면 자동 미렌더). `GoldPremiumWidget` 아래 **`RiskCalculatorCard`**(접힘형 풀폭). **Phase 5(신호 다이어트): `GuruSignalCard`는 항상 상단 위치 유지** — `showGuruSignalsProminently`가 true면 기존처럼 펼쳐 크게, false(기본)면 **같은 자리에서 접이식**(`collapsible`+`defaultCollapsed`, storageKey `asset-manager-guru-card-open`). 바로 아래 **`ReferenceIndicatorsSection`**(참고 지표, 구루 카드 제외·리스크 매트릭스만). **`ProfitLossChart`는 접이식 기본 접힘**(collapsible+defaultCollapsed, storageKey `asset-manager-profitloss-open`). **`TopBottomAssets`(자산별 성과 요약)는 대시보드에서 렌더 제거**(컴포넌트/훅은 보존). 계산/차트/발화 무변경(표시·레이아웃만) | `PortfolioContext`, `useGlobalPeriodDays`, `GoldPremiumWidget`, `MarketDistributionBanner`, `RiskCalculatorCard`, `GuruSignalCard`, `ReferenceIndicatorsSection`, `ProfitLossChart`, `AllocationChart`, `CategorySummaryTable`, `RebalancingTable` |

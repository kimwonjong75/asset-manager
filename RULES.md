...
| `knowledgeIngest.ts` | **인제스트 승인 큐 처리 (순수 함수)**. `parseIngestQueue(jsonl)` → `IngestQueueEntry[]`(형식 불일치 줄 스킵). **`canPromoteRule(rule, kb, implemented?)`** → `PromoteCheck{ok, blockers[]}` — `rule.status='active'`(신호 활성화) 직전 무결성 검사: 연결 claim 실재·미거부(rejected)·sourceId 보유, rule.verification 게이트(userApproved‖dataVerified‖backtestVerified), **signal 규칙은 `IMPLEMENTED_METRICS`만**(미구현이면 차단). `isActiveSignal`이 rule만 게이트하므로 이 함수가 연결 claim 무결성의 **마지막 문지기**. `applyApproval(kb, entry, opts, today)` — 승인 후보 upsert(claim: userApproved=true + pending-ingest 태그 제거 / rule: activate&&canPromote면 status='active' 아니면 'draft'). `IMPLEMENTED_METRICS`는 guruSignalEngine.buildMetricValues·`scripts/ingest/triage_commit.py`와 동기 필요. side effect/any 없음 | `useKnowledgeInbox` |
...
| `InvestmentGuideView.tsx` | 투자 가이드 탭. **첫 섹션 `inbox`('지식 인제스트')에 `KnowledgeInboxPanel`(승인 큐) 장착** — 나머지 섹션(매매시그널/MA/RSI/필터/실전/원칙)은 정적 교육 UI. **`SECTIONS`에 항목 추가 시 `Icons`/`SECTION_COLORS`(Record<SectionId>)도 동기 필요** (안 그러면 타입 에러) | `KnowledgeInboxPanel` |
...
| 파일 | 책임 | 의존 |
|------|------|------|
| `WatchlistAddModal.tsx` | 관심종목 추가 모달. **종목검색 실패 시 `searchError` state로 사유 표시**(`searchSymbols`의 `SymbolSearchError.message`), 결과 0건이면 "검색 결과가 없습니다" 표시 | `PortfolioContext` (`addWatchItemOpen`, `addWatchItem`) |
...
| 파일 | 책임 | 의존 |
|------|------|------|
| `AddNewAssetModal.tsx` | 새 자산 추가 모달 (심볼 검색, 카테고리 추론, 중복 검사). **종목검색 실패 시 `searchError` state로 사유 표시**(`searchSymbols`의 `SymbolSearchError.message`), 결과 0건이면 "검색 결과가 없습니다" 표시. **배치 B**: 매수일자 아래에 `PositionSizingCalculator`(진입가=매수가, 통화=선택 통화, 암호화폐면 소수 허용) → "이 수량 적용"으로 수량 자동 입력 | `geminiService`, `PortfolioContext`, `CategoryDefinition`, `PositionSizingCalculator` |

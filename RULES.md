...
| `actionQueueGenerator.ts` | **터틀 주문 생성기 (Phase 2a, 90/10, 순수 함수)** — 터틀 포지션+진입후보+`TurtleMarketInput`(종목별 계산된 N·돈치안·환율, 호출부가 OHLCV로 산출 주입)를 받아 실행 큐 `ActionItem[]` 생성. **`buildTurtleActions(input)`** — 오픈 포지션은 **손절>청산>피라미딩** 우선순위(`evaluateStop/Exit/Pyramid` 위임), 진입 후보는 `evaluateEntry` — **배치 예산·리스크 누적**(채택분 riskKRW·positionValueKRW를 다음 후보 판정에 반영해 12% 한도·예산 초과 진입 차단). **중복 방지**: 이미 대기(pending/snoozed)인 `(kind\|ticker)` 재생성 안 함(`dedupKey`). **fail-closed**: 시장입력 없음·N null·미돌파·기보유 종목이면 미생성. `today`·`makeId(seq)` 주입(순수·결정성, Date.now 없음). 파생 헬퍼: `actionDaysIgnored`/`actionEscalationLevel`(3일+ or 스누즈2회+=1, 7일+ or 스누즈3회+=2)/`isSnoozeExpired`. 통화 규약(D6): price/N/donchian 원통화, 금액 KRW. 회귀 `tests/actionQueueGeneratorParity.ts`(32단언, `npm run test:actionqueue`) | `turtleEngine`, `types/turtle`, `types/actionQueue` ← (Phase 2b useActionQueue), `tests/actionQueueGeneratorParity` |
...
| `actionQueueGeneratorParity.ts` | **터틀 주문 생성기(Phase 2a) 골든 테스트**(32 단언). ① 진입 생성(돌파)·미돌파/데이터결손/N null/기보유 미생성 ② 손절>청산>피라미딩 우선순위·최대유닛 도달 미생성 ③ 중복 방지(pending/snoozed 재생성 안 함, done은 재생성) ④ 배치 리스크 누적(12% 한도 15후보 중 12건)·예산 잔여 바인딩(1건) ⑤ 파생 헬퍼(daysIgnored/escalation/snooze만료). 운영 함수만 호출 | `npm run test:actionqueue` |

...
| `useSignalReplay.ts` | **신호 리플레이 오케스트레이션** — OHLCV fetch(주식/코인 분기, 10년, requestId 레이스 가드, 실패 폴백) + 종목검색(debounce 250ms) + 타임라인 계산(`buildReplayTimeline`, 한 틱 양보 후 동기 빌드 — 252일/단일종목 기준 충분, 느리면 후속 청크화) + as-of 네비(slider/click/이전·다음 날/이전·다음 신호/최신) + 모드(replay/review)·윈도(6M/1Y/2Y/3Y) + 샌드박스 오버라이드 state(P3 UI 연결, **1차 빈 상태**). **P2 판정/사례**: `verdictFor/setVerdict/clearVerdict`(SignalVerdict upsert·삭제, **localStorage `-verdicts-v1`**, 종목·날짜·ruleId? 키)·`verdictDates`(구분표시)·`tickerVerdicts` + `saveCurrentCase(role,memo)`(현재 타임라인→`buildVerificationCase`, id=`Date.now`+seq, **localStorage `-cases-v1`**)·`loadCase`(종목·`anchorDate`·윈도 세팅 후 재실행)·`deleteCase`·`comparingCase`/`caseDiff`(저장당시 vs 재실행 `diffCaseResults`, **종목·윈도 일치 시만**)·`endComparison`. **`anchorDate` settable**(loadCase가 과거 종료일 고정→재현; selectSymbol/endComparison이 undefined로 리셋=최신). **윈도 변경·종목 변경 시 비교 종료**. **유효 규칙=`applyRuleOverrides(knowledgeBase.rules, sandbox)`(1차 영구 오버라이드 없음). 라이브 구루 신호/KnowledgeBase/Drive 절대 불변** | `signalReplay`, `ruleOverrides`, `replayVerdicts`, `replayCases`, `historicalPriceService`, `symbolListService`, `PortfolioContext`(data.knowledgeBase, ui.alertSettings.rules) | `SignalReplayView` |
...
| `replayCases.ts` | **검증 사례 저장/diff(P2, 순수 + 얇은 localStorage 래퍼)** — 키=`asset-manager-replay-cases-v1`. **순수**: `collectPerRuleResults(days)`(구루 eligible&&matched만 규칙별 발화일=마커 게이팅 동일 기준), `buildRuleSnapshot`(signal 규칙만 conditionJson+leafIds, 시드 변해도 재현), `computeResultMetrics`(signalCount+신호일 avgRet20), `buildVerificationCase`(**id/createdAt 호출부 주입=순수**, rulesetHash=`hashRuleset`, **verdicts는 `timeline.days` 날짜 Set으로 필터 → 사례 기간 밖 판정 오염 차단**), `diffSignalDates(prev,next)`→{added,removed}, `diffCaseResults`(전체+규칙별, 변화 없는 규칙 제외), `upsertCase`(같은 id 교체·없으면 **최신 우선 prepend**)/`removeCase`/`parseCases`(필수필드 없으면 폐기). **부수효과**: `loadCases`/`saveCases`. **caseRole(research|holdout)** — holdout은 P3 과적합 방지 게이트. **overridesSnapshot은 P2에서 항상 []**. Drive 미동기화 | `utils/logger`, `conditionLeafId`(flattenLeaves), `ruleOverrides`(hashRuleset), `types/{knowledge,signalReplay}` ← `useSignalReplay`, `tests/signalReplayParity` |
...
| `SignalReplayView.tsx` | **"신호 리플레이" 탭** (신규 `'replay'` 탭) — 구루 신호를 과거 종목에 대입해 매수/매도 타이밍 검증(1차 연구도구). 종목 picker(보유/관심 퀵픽 + 검색) + 모드 토글(replay 미래가림/review 복기) + 윈도 + 전용 차트(`SignalReplayChart`: 외부 OHLCV 주입·자체 fetch/오늘현재가 없음·replay 모드 asOf 이후 가림·`createSeriesMarkers` 마커·`subscribeClick` 이동) + 슬라이더/이전·다음 날·이전·다음 신호 + **구루 진단(중심): `describeRuleStatus` + leaf별 실제값/기준/통과 + 근접도(boundaryDistance)로 "왜 떴나/왜 안 떴나"** + 신호 후 성과(5/20/60일·최대상승/하락) + **가격기반 알림은 "참고용"**(수익률/손절 등 보유가 기준 필터 미발화·서버신호 재현불가 명시) + **P2 판정 패널(`ReplayVerdictPanel`)·사례 패널(`ReplayCasesPanel`)** + 시점 네비바 판정 뱃지(`VERDICT_KIND_LABELS`). **사례 패널은 종목 미선택 상태에서도 항상 노출**(저장 사례 재실행 진입점). 면책: "현재 활성 규칙을 과거 가격에 재적용·실제 발송 알림 아님". **라이브 구루 신호/기존 알림 불변** | `useSignalReplay`, `components/replay/{SignalReplayChart,ReplayVerdictPanel,ReplayCasesPanel}`, `guruDiagnostics`(describeRuleStatus), `alertDiagnostics`(describeAlertRuleStatus), `chartFormat`, `PortfolioContext` |
...
| `replay/ReplayCasesPanel.tsx` | **검증 사례 패널(P2, 렌더 전용)** — caseRole 토글(연구용/검증용 holdout) + 메모 + "현재 화면 저장"(`canSave`=타임라인 존재 시) / 사례 목록(종목·기간·역할 뱃지·신호일수·평균20일후·판정수·메모, **재실행/삭제**) / **재실행 비교 diff**(comparingCase 활성 시 added/removed 신호일 전체+규칙별, 변화없음=동일재현 안내, "비교 종료"). 모든 동작은 훅 콜백 | `types/signalReplay`(VerificationCase/ReplayCaseRole), `utils/replayCases`(CaseDiff 타입) ← `SignalReplayView` |
...
| `tests/signalReplayParity.ts` | 독립 실행 (`npm run test:replay`, 83단언). 신호 리플레이 회귀 — **룩어헤드 방지**(full vs asOf까지 자른 시리즈 진단 동일, outcome만 상이) + `boundaryDistance`(between/음수/0) + `applyRuleOverrides`(leafId 패치·enabled=false 제거·비파괴) + `flattenLeaves` 안정 id + `buildReplayTimeline` 동작 + **마커 구루 게이팅**(markers.guruCount>0·signalDates가 구루 마커로만 뒷받침) + **P2 판정 저장**(verdictKey 키구분·upsert 수정/추가/비파괴·놓친매수 비신호일 태깅·삭제 day vs rule 분리·안전파싱) + **P2 사례**(collectPerRuleResults eligible&&matched만·computeResultMetrics avgRet20·buildVerificationCase rulesetHash 일치·**기간 밖 판정 필터·규칙별 판정 보존**·diffSignalDates/diffCaseResults added/removed·upsertCase 최신우선·removeCase·parseCases 폐기). 합성 OHLCV(네트워크 없음). 운영 함수만 호출(계산식 복제 금지) |

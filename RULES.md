...
| 파일 | 책임 | 의존 |
|------|------|------|
| `AddNewAssetModal.tsx` | 새 자산 추가 모달 (심볼 검색, 카테고리 추론, 중복 검사). **키리스 검색**(`symbolListService.searchSymbols`)이 기본, 실패 시 `searchError`로 사유 표시. 검색 패널(`showSearchPanel`): 결과 리스트 + "✏ 티커로 직접 추가"(`handleManualAdd` — **`validateTicker`로 시세 검증 후에만 확정**, 실패 시 searchError) + **키 있을 때만** "✨ AI로 더 찾기"(`handleAiSearch`→`searchSymbolsAI` 결과 병합). **배치 B**: 매수일자 아래에 `PositionSizingCalculator`(진입가=매수가, 통화=선택 통화, 암호화폐면 소수 허용) → "이 수량 적용"으로 수량 자동 입력. **자산 구분 아래에 전략 버킷 토글(코어/투더문) + 계정 토글(원종/유선)** — 기본 코어·원종, onAddAsset 페이로드에 `bucket`·`owner` 전달 | `symbolListService`(searchSymbols/validateTicker), `geminiService`(searchSymbolsAI), `geminiSettings`(getGeminiApiKey), `PortfolioContext`, `CategoryDefinition`, `PositionSizingCalculator`, `types/bucket`, `types/owner` |
| `EditAssetModal.tsx` | 자산 편집 모달 (isDirty 닫기 보호 적용, 티커 재검색은 `symbolListService.searchSymbols` 키리스). **전략 버킷 토글(코어/투더문) + 계정 토글(원종/유선) 포함** — `getAssetBucket`/`getAssetOwner`로 현재값 표시, isDirty 비교에 버킷·계정 변경 포함 | `symbolListService`, `PortfolioContext`, `CategoryDefinition`, `types/bucket`, `types/owner` |

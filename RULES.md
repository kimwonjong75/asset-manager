...
| `guruDiagnostics.ts` | **구루 규칙 진단 (5A, 순수 함수)** — "왜 안 떴나"를 **3축 직교** 분해. `diagnoseRule(rule, claims, metrics, ohlcvAvailable, now)` / `diagnoseAssetRules({rules, claims, target, now})`(computability='signal'만, `buildMetricValues` 1회 산출 공유) → `RuleDiagnostic{eligibility{eligible,reasons}, evaluation(matched/unmatched/unknown/not-evaluated), coverage(MetricCoverage[]), leaves}`. **eligibility=`getSignalEligibility`(+'no-condition')로 getActiveSignalRules 멤버십과 일치** → `diag.filter(eligible&&matched)` === `evaluateGuruSignals`(**매치셋 동일성**, 테스트 가드). **coverage는 조건 leaf 기준**, `classifyMetricAvailability`: unsupported(IMPLEMENTED_METRICS 외)/missing(값 없음)/**partial**(climaxFlags·distributionCount이 ohlcvAvailable=false: 0 degrade이지 missing 아님)/available. `summarizeDiagnostics`→**3축 독립 카운트**(eligibility/evaluation/readiness collapse 안 함 — partial이 미충족에 숨지 않음), `ruleReadiness`(규칙당 최악 availability; **조건 없으면 `not-applicable`** — complete 오분류 방지). **`evaluateGuruSignals` 미변경(additive)**. 회귀 `tests/guruDiagnosticsParity.ts` | `knowledgeScoring`(getSignalEligibility), `guruSignalEngine`(evaluateCondition/buildMetricValues), `conditionDescribe`(explainConditionLeaves), `types/knowledge`(IMPLEMENTED_METRICS) → `GuruSignalCard`(진단 패널, 5A-⑤) |

...
| `alertRules.ts` | 알림 규칙 타입 (`AlertRule`, `AlertResult`, `AlertSettings`, `AlertMatchedAsset`). `AlertMatchedAsset`에 `source?: 'portfolio' \| 'watchlist'` 필드 포함. `AlertRuleFilterConfig`에 `maCrossPeriod?`, `withinDays?`, `maxLookbackTradingDays?`, **클라이맥스 임계값 3개**(`climaxFlagsRequired?`/`climaxSlopeMultiplier?`/`climaxAtrMultiple?`), **클라이맥스 게이팅 토글 2개**(`climaxRequireBullishCandle?`/`climaxRequireLongTrendUp?` — 둘 다 기본 true), **디스트리뷰션 임계값 3개**(`distributionWindow?`/`distributionVolumeRatio?`/`distributionThreshold?`) 포함 | `constants/alertRules`, `utils/alertChecker`, `hooks/useAutoAlert`, `AlertSettingsPage`, `AlertPopup` |

...
| 파일 | 책임 | 의존 |
|------|------|------|
| `DashboardView.tsx` | 대시보드 탭 — 최상단에 `MarketDistributionBanner` (시장 지수 디스트리뷰션 경고, 모든 지수 safe면 자동 미렌더). `GoldPremiumWidget` 아래 **`RiskCalculatorCard`**(접힘형 풀폭). **Phase 5(신호 다이어트): `GuruSignalCard`는 `ui.signalDisplay.showGuruSignalsProminently`가 true일 때만 상단 큰 카드로 렌더**, 기본(false)이면 최하단 **`ReferenceIndicatorsSection`**(참고 지표 접힘 섹션) 안으로 강등. 계산/발화는 무변경(표시 위치만) | `PortfolioContext`, `useGlobalPeriodDays`, `GoldPremiumWidget`, `MarketDistributionBanner`, `RiskCalculatorCard`, `GuruSignalCard`, `ReferenceIndicatorsSection` |
...
| `ReferenceIndicatorsSection.tsx` | **참고 지표 접힘 섹션 (Phase 5 — 신호 다이어트, 렌더 전용)** — 대시보드 최하단. 예측형 참고 신호를 기본 화면에서 강등하는 컨테이너. 펼침/접힘 상태 localStorage(`asset-manager-reference-indicators-open`, 기본 접힘 — `RiskCalculatorCard` 패턴). 내부: 항상 **`RiskMatrixPanel`**(컴팩트 리스크 매트릭스) + **`ui.signalDisplay.showGuruSignalsProminently`가 false일 때만 `GuruSignalCard`**(true면 DashboardView가 상단에 렌더하므로 중복 방지 제외). 헤더 문구 "실행할 주문은 실행 큐 기준". **접힘 헤더 우측 muted 카운트 배지**(과열 N=`riskMatrix` 티어 있는 수·구루 N=`guruSignals` distinct assetId 수, 강등 유지하되 완전 미숨김 — 펼치지 않고 훑기용, 표시 전용). 계산/발화 무접촉 | `PortfolioContext`(ui.signalDisplay/derived.riskMatrix/guruSignals), `GuruSignalCard`, `RiskMatrixPanel`, `lucide-react`(ChevronDown) ← `DashboardView` |

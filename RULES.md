...
| `GuruSignalCard.tsx` | **⑤ 가시화** — 대시보드 "구루 신호 엔진" 카드(풀폭, `DashboardView`에서 단독 스택). **좌(신호 리스트)/우(선택 종목 차트) 분할**(`lg:flex`, 모바일은 차트가 리스트 아래로 쌓임). 좌측: `derived.guruSignals`를 **`groupGuruSignals`로 액션별→종목별 묶어** 렌더(같은 종목의 여러 규칙은 칩으로 한 줄 — 중복 줄 방지). 헤더 "활성 규칙 N개 평가" + "{종목수}개 종목·{신호수}개 신호" 요약, 그룹별 종목수(중복 시 신호수 병기). 종목 행: 이름/티커/관심뱃지 + 규칙 짧은라벨 칩(`RULE_SHORT_LABELS`) + "ⓘ 무효화:" 줄(rule.riskPolicy dedup). **행 클릭 → `selectedAssetId` 갱신**(선택 행 `ring-primary/60` 하이라이트), 미선택/사라진 종목이면 **우선순위 최상위(첫 그룹·첫 종목) 폴백**. 우측: `derived.guruSignalChartTargets[assetId]`로 `AssetTrendChart` 인라인 렌더(`key={assetId}`로 종목 전환 시 재마운트), ⤢는 공용 `ChartViewerModal` 전체화면. **차트 props의 source 분기 룩업은 컴포넌트가 아닌 `guruSignalEngine.buildGuruSignalChartTargets`(순수)→context derived에서 수행**(RULES: 컴포넌트는 렌더만). **폰트**: 설명/카운트류는 `text-xs`(12px), 칩·무효화·관심뱃지는 `text-[11px]`(이전 10px에서 +1px). **문구 안전장치**: 부제 "관찰 후보 · 매수 추천이 아닙니다" + 면책. 매칭 0건/활성 0개 분기. 평가·그룹핑은 `guruSignalEngine`(순수) 위임, 렌더만 담당. 현재 active+condition 4개(climax-top-sell 매도경고 + pullback/near-high/reclaim 관찰후보). rsRank 구현 시 엄격 RS 규칙 추가 확장 | `PortfolioContext`(derived.guruSignals/guruSignalChartTargets + data.knowledgeBase/portfolioHistory), `guruSignalEngine`(getActiveSignalRules, groupGuruSignals), `AssetTrendChart`, `ChartViewerModal`, `types/knowledge`(RuleAction) |

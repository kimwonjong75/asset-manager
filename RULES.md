...
| `smartFilterLogic.ts` | 스마트 필터 매칭 (그룹 내 OR, 그룹 간 AND), enriched 지표 참조, `PRICE_BELOW_*` 판정 포함. 거래량 필터(`VOLUME_SURGE/HIGH/LOW`)는 `indicators.volume_ratio` 사용. **골든/데드크로스 필터는 상태 기반** (`MA_GOLDEN_CROSS`: shortMA > longMA, `MA_DEAD_CROSS`: shortMA < longMA — 교차 당일뿐 아니라 지속 상태 감지, 경과일은 UI `CrossDaysBadge`에서 표시(60거래일 초과는 숨김, `scaleY(0.8)` 축소 뱃지, 종목명 오른쪽 컬럼에 배치). **뱃지가 사용하는 MA 페어는 `alertSettings.rules`의 `golden-cross`/`dead-cross` 룰의 `filterConfig.maShort/LongPeriod`를 직접 참조** (PortfolioTable에서 추출 → `BadgePairs`로 전달). 두 룰 페어가 다를 수 있어 한 자산이 GC/DC 신호를 동시에 가질 수 있고 활성 신호 모두 표시. 룰이 `enabled=false`면 해당 신호 미표시). **`matchesSingleFilter()` export** — 알림 규칙 체커에서도 재활용 | `PortfolioTable`, `alertChecker.ts` |

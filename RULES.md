...
| 파일 | 책임 | 의존 |
|------|------|------|
| `DashboardView.tsx` | 대시보드 탭 — 최상단에 `MarketDistributionBanner` (시장 지수 디스트리뷰션 경고, 모든 지수 safe면 자동 미렌더). `GoldPremiumWidget` 아래 2열 그리드로 **`RiskCalculatorCard`**(리스크 계산기) + **`GuruSignalCard`**(구루 신호 엔진) 배치 | `PortfolioContext`, `useGlobalPeriodDays`, `GoldPremiumWidget`, `MarketDistributionBanner`, `RiskCalculatorCard`, `GuruSignalCard` |
...
| `ChartViewerModal.tsx` | 개별 차트 전체화면 뷰어 — `createPortal`(document.body)로 `AssetTrendChart`를 `fillParent`로 크게 렌더. Esc·백드롭 클릭 닫기, 열린 동안 body 스크롤 잠금, 우상단 X 닫기 버튼. Props: `AssetTrendChartProps` + `onClose`. **포트폴리오 테이블/모바일카드/관심종목(테이블·모바일카드) 4곳 공용** — "차트 확대" 메뉴 또는 인라인 차트 헤더의 ⤢ 아이콘(`onExpand`)으로 호출. `useHistoricalPriceData` 모듈 캐시(10분 TTL) 덕에 인라인 차트 본 종목은 재fetch 없음 | `react-dom/createPortal`, `AssetTrendChart`, `lucide-react`(X) |
...
| 파일 | 책임 | 의존 |
|------|------|------|
| `AddNewAssetModal.tsx` | 새 자산 추가 모달 (심볼 검색, 카테고리 추론, 중복 검사). **배치 B**: 매수일자 아래에 `PositionSizingCalculator`(진입가=매수가, 통화=선택 통화, 암호화폐면 소수 허용) → "이 수량 적용"으로 수량 자동 입력 | `geminiService`, `PortfolioContext`, `CategoryDefinition`, `PositionSizingCalculator` |
...
| `GuruSignalCard.tsx` | **⑤ 가시화** — 대시보드 "구루 신호 엔진" 카드. `derived.guruSignals`를 **`groupGuruSignals`로 액션별→종목별 묶어** 렌더(같은 종목의 여러 규칙은 칩으로 한 줄 — 중복 줄 방지). 헤더 "활성 규칙 N개 평가" + "{종목수}개 종목·{신호수}개 신호" 요약, 그룹별 종목수(중복 시 신호수 병기). 종목 행: 이름/티커/관심뱃지 + 규칙 짧은라벨 칩(`RULE_SHORT_LABELS`) + "ⓘ 무효화:" 줄(rule.riskPolicy dedup). **문구 안전장치**: 부제 "관찰 후보 · 매수 추천이 아닙니다" + 면책. 매칭 0건/활성 0개 분기. 평가·그룹핑은 `guruSignalEngine`(순수) 위임, 렌더만 담당. 현재 active+condition 4개(climax-top-sell 매도경고 + pullback/near-high/reclaim 관찰후보). rsRank 구현 시 엄격 RS 규칙 추가 확장 | `PortfolioContext`(derived.guruSignals + data.knowledgeBase), `guruSignalEngine`(getActiveSignalRules, groupGuruSignals), `types/knowledge`(RuleAction) |
| `RiskCalculatorCard.tsx` | **배치 C** — 대시보드 standalone "리스크 계산기" 카드. `derived.totalValue`(총자산 KRW) 자동주입(수정 가능), 진입가 통화 선택(KRW/USD/JPY 토글), `entryPrice={null}`로 자체 입력 모드 → 공용 `PositionSizingCalculator`에 위임. 매수 없이 평상시 적정 수량 가늠용 | `PortfolioContext`, `PositionSizingCalculator`, `Currency` |
